...
Результат: Cohen's Kappa Test: 0.615, AUC-ROC: 0.999 (набор данных очень маленький, поэтому результат нельзя трактовать прямо, как 100 % качество модели, но практическое использование показало, что результат хороший). "3.1. Тренировка модели\TestResultsOnRStudio.png"
...
| Модель 1А RandomForest | 0.615 | Набор данных маленький, поэтому нельзя делать вывод, что модель обеспечивает 100 % качество, но результат хороший, но несколько шумный (есть явно лишние акции). |
