...
Модель 1А: тренирована на shares_detector_train.csv, shares_detector_test (Результат: "3.1. Тренировка модели\TestResultsOnRStudio.png"). Точный набор данных при тренировке первых моделей не сохранил (файл затерся новыми обновлениями), но приблизительно воспроизвел.
...
Результат:
- Cohen's Kappa Train: 0.969, Test: 1, AUC-ROC: 1 (набор данных очень маленький, поэтому результат нельзя трактовать прямо, как 100 % качество модели, но практическое использование показало, что результат хороший). "3.1. Тренировка модели\TestResults(ext)OnRStudio"
